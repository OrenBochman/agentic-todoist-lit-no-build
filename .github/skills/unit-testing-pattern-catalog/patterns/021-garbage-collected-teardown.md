---
title: Garbage-Collected Teardown
description: Refactor unnecessary cleanup away when only memory objects are left behind.
keywords: [teardown, memory-only, cleanup, unit]
---

## Action List
1. Identify the current test or fixture problem that matches this pattern.
2. Apply a small refactoring to refactor unnecessary cleanup away when only memory objects are left behind.
...
## When to use
Use when the test problem is well described by: unnecessary cleanup away when only memory objects are left behind
...
## Prefer over
[Automated Teardown](./003-automated-teardown.md)
...
## Notes
See also [Automated Teardown](./003-automated-teardown.md).
