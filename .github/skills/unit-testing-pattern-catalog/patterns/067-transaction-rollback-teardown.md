---
title: Transaction Rollback Teardown
description: Refactor persistent database cleanup by wrapping test writes in a rollbackable transaction.
keywords: [transaction-rollback, database, cleanup, integration]
---

## Action List
1. Identify the current test or fixture problem that matches this pattern.
2. Apply a small refactoring to refactor persistent database cleanup by wrapping test writes in a rollbackable transaction.
...
## When to use
Use when database changes can be safely undone by rolling back the test transaction.
...
## Prefer over
[Table Truncation Teardown](./046-table-truncation-teardown.md)
...
## Notes
See also [Table Truncation Teardown](./046-table-truncation-teardown.md).
