...
description: Refactor blocked verification into a small subclass used only by tests.
keywords: [subclass, seam, observability, unit]
---

## Action List
1. Identify the current test or fixture problem that matches this pattern.
2. Apply a small refactoring to refactor blocked verification into a small subclass used only by tests.
...
## When to use
Use when the test problem is well described by: blocked verification into a small subclass used only by tests
