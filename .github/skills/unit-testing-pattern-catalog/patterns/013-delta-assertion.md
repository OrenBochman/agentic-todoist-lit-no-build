---
title: Delta Assertion
description: Refactor absolute-state checks into assertions about the change caused by the action.
keywords: [delta, before-after, balance, counter, unit]
---

## Action List
1. Identify the current test or fixture problem that matches this pattern.
2. Apply a small refactoring to refactor absolute-state checks into assertions about the change caused by the action.
...
## When to use
Use when the test problem is well described by: absolute-state checks into assertions about the change caused by the action
